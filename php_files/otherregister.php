		...
		if($db->checkifteacherexisted($username)){
			...
			if($isValid){
				if ($password == $confirmpassword) {
					$user = $db->storeTeachersData($fullname, $username, $email, $password, $confirmpassword); //create new user
				   if($user){
						...
						echo json_encode($response);
						$verify = $db->sendemailverifyforteacher($email, $username, $password); //send email for verification
					}else{
