<?php
	...
	if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['teacherid'])) {
		...
		$verify = $db->checkteacheractived($username); //check if email verification is over or not
		$checkuser = $db->checkifteacherexisted($username);
		if($checkuser != false){
			...
			if($verify){
				if(!$db->checkifteacheridgrexisted($teacherid)){
					...
					$user = $db->getteacherData($username, $password, $teacherid); // get username & password
					if($user != false){
